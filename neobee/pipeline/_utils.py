...
def _make_progress(session_id: str, stage: str, task_id: int | None = None):
    """Create a progress callback that wraps tracker.update_progress.

    Usage in pipeline nodes::

        progress = _make_progress(session.id, "stage_name", state.get("task_id"))
    ...
    def _progress(pct: int, step: str) -> None:
        if tracker and task_id:
            tracker.update_progress(session_id, stage, task_id, pct, step)
