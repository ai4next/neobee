...
import type { CreateSessionInput } from '@neobee/shared';
...
// Start all stage controllers
const orchestrator = new StageOrchestrator(store, eventBus);
orchestrator.startAll();

// Wire task progress to event bus
taskTracker.setProgressListener((progress) => {
  eventBus.emitRaw(progress.sessionId, 'task.progress', progress.stage, {
...
app.get('/api/sessions/:id/tasks/:stage', (req, res) => {
  try {
    ...
    const result = sessionsService.getTaskWithSteps(id, stage as any, page, pageSize);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : 'Unknown error' });
  }
